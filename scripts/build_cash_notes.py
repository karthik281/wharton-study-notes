"""Build one Obsidian study-notes .md for the Cash Constrained module.

Inputs: all transcripts in <folder>/transcripts/*.txt + the Cash Constrained slides PDF.
Output: <folder>/Cash Constrained.md  -- YAML frontmatter + MOC backlink + structured
notes with [[wikilinks]] to existing Concepts/ notes.
...
FOLDER = Path(r"C:\Users\raoka\Documents\WEMBA\Term 4\OIDD 6360 - Scaling Operations"
              r"\05 Async - Constrained\Cash Constrained")
...
module's slide deck. Produce comprehensive, faithful study notes in Markdown — study
notes, not a summary: capture everything of substance so the student never has to
re-watch the videos.
...
- Preserve specific terminology, numbers, named cases, and named people. The transcripts are
  ...
  source of truth (e.g. "bleed scaling" -> "Blitzscaling", company/person names, financial terms).
- Reconcile the audio with the slides: where the slides give exact figures or tables, use them.
...
2. Then a sequence of thematic ## sections following the lecture's arc, with rich ### subsections,
   tables for any structured data (assumptions, sensitivity tables, financials), and worked numbers.
...
def main() -> None:
    ...
    parts = [f"**Course:** {COURSE}", f"**Module:** {TITLE} (async — Module 5 Part I)", ""]
    ...
    for pdf in pdfs:
        ...
        if txt:
            parts.append(f"## Slides: {pdf.name}\n{txt[:80000]}\n")

    parts.append("## Existing concept-note names (use these for [[wikilinks]] when mentioned):\n"
                 + ", ".join(concept_names))

    user_content = "\n".join(parts)
    print(f"Transcripts: {len(tfiles)} | slides: {len(pdfs)} | concepts: {len(concept_names)} "
    ...
    for m in re.findall(r"\[\[([^\]]+)\]\]", body):
        ...
        if name and name != "OIDD 6360 - MOC" and name not in links:
            links.append(name)

    materials = [pdf.name for pdf in pdfs] + [
        "REffP Case.pdf", "REffP Base Year (1).xlsx", "REffP Scaling Model.xlsx",
        "SaaS model Students Version - Solution.xlsx",
    ]
    fm = (
        "---\n"
        f'title: "{TITLE}"\n'
        ...
        "session: 5\n"
        "type: session-note\n"
        "tags: [oidd-6360, async, constrained, cash-constraints, saas-modeling, "
        "burn-rate, blitzscaling, financial-modeling]\n"
        f"concepts: [{', '.join(links)}]\n"
        f"materials: [{', '.join(materials)}]\n"
        "---\n\n"
        f"# {TITLE}\n\n"
        "> Part of [[OIDD 6360 - MOC]] · sibling of [[05 Async - Constrained]] "
        "(Resource Constrained covered there)\n\n---\n\n"
    )
